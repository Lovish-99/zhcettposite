...
const JobList = () => {
  ...
  const getProfiles = async () => {
    if (auth) {
      setLoading(true);
      const id = JSON.parse(localStorage.getItem("recruiter"))._id;
      let result = await fetch(
        `http://localhost:5000/jobapi/get-job-recruiter/${id}`,
        {
          headers: {
                ...
                <MDBCardBody style={{ backgroundColor: "	#b2d8d8" }}>
                  <MDBRow>
                    ...
                    <MDBCol>
                      <MDBRow>
                        <Link
                          to={`${item._id}/${item.position}`}
                          style={{ textDecoration: "none" }}
                        >
                          <MDBBtn type="submit"> View Applies</MDBBtn>
                        </Link>
                      </MDBRow>
